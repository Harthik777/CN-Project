...
The Pages artifact commit is `09dba20099eb769f15d8bbb54457a7b41d60f47e`. Attempt 1 of the [publishing run](https://github.com/Harthik777/CN-Project/actions/runs/34470262202) passed its build, then timed out after ten minutes in GitHub's `updating_pages` phase. The failed publishing job was retried. At this record's check, attempt 2 was still running and the public HTML did not match the new artifact; **public deployment of v4.3.0 is not yet verified**. The downloadable release is available independently of that deployment. The earlier Pages attempt for the intermediate artifact was cancelled when the final artifact superseded it.

The [local HTTP artifact check](experiments/standalone-local-4.3.0.json) passed. The public artifact verifier (`scripts/verify_standalone_demo.py`) contacts only Pages, checks exact HTML bytes and the matching offline worker, and verifies the absence of backend routes. Run it after Pages finishes, or dispatch **Verify standalone demo** against `codex/full-stack`, before claiming the public update complete. Offline use requires a successful initial visit or a saved presentation file; browser storage can be evicted. Model training and real-data limitations are unchanged.
